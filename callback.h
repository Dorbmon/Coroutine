...
namespace RCo {
    ...
    public:
        ...
        static void await_suspend(std::coroutine_handle<RPromise<Result, IN_OP_T>> h,
            ...
            auto f = static_cast<std::function<void(std::unique_ptr<std::function<void(Result&&)>>)>*>(metadata);
            
            // Capture the function, coroutine handle, and worker in a lambda
            // Ensure that the function pointer is deleted after use to prevent memory leaks
    ...
    RTask<Ret_V, CallbackOP> callback_to_awaitable(Function&& f) {
        auto promise = (typename RTask<Ret_V, CallbackOP>::promise_type) {};
        auto f_p = new std::function<void(std::unique_ptr<std::function<void(Ret_V&&)>>)>(f);
